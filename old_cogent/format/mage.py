...
Written 6/12/03 by Rob Knight

7/21/03 Rob Knight: explicitly set MinRadius and DefaultRadius as properties
of MagePoint class instead of using arbitrary constants. New behavior is to
insert DefaultRadius if self.Radius < MinRadius when stringifying the point.
This works around a bug in MAGE where it does not display small radii
correctly.

Fixed bug where radius was _always_ set to a default value even if it was
passed as None (which had caused unit tests to fail). Now complies with
unit tests again.

10/28/03 Rob Knight: merged into PyEvolve. Moved some methods of MagePoint to
alternative constructors. Added cmp for MagePoint (compares all fields).

10/30/03 Rob Knight: finished MagePointFromBaseFreqs, adding functions to
compute presentation attributes on the fly.

8/16/04 Sandra Smit: Implemented inheritance from build-in __list__ for 
MageList and MageGroup. Removed Cascader functionality. Still allows for Group
initialization with list properties; they will be used for all groups and lists
that don't specify that property already. Added iterators for points, lists, 
groups, and groupsAndLists. 

8/19/04 Sandra Smit: Added toCartesian and fromCartesian functionality.

9/20/04 Sandra Smit: Added lookup function for _set_coord and _get_coord to 
MagePoint. Changed value check in MagePoint toCartesian() and MageList
toCartesian().
2/23/06 Sandra Smit: modified toCartesian and fromCartesian to use 
fix_rounding_error.
"""
...
from old_cogent.util.misc import extract_delimited
...
class MageList(list):
    ...
    def toCartesian(self):
        ...
        for point in self.iterPoints():
            result.append(point.toCartesian())
        return result
...
class MageGroup(list):
    ...
    def toCartesian(self):
        ...
        for item in self:
            result.append(item.toCartesian())
        return result
...
class Kinemage(object):
    ...
    """
    def __init__(self,Count=None,Header=None,Groups=None,Caption=None,Text=None):
    ...
    def toCartesian(self):
        ...
        for item in self.Groups:
            result.Groups.append(item.toCartesian())
        return result
