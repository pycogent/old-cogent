#!/usr/bin/env python
#file evo/stats/distribution.py
...
4/21/03 Rob Knight: started translating methods.

4/29/03 Rob Knight: support for z, t, and chi-sqared tests now complete. 
Interfaces for tests are x_low, x_high, and xprob (the last of these
being 2-tailed). docstrings now up to date.

5/8/03 Rob Knight: added missing MINLOG constant

8/4/03 Rob Knight: changed name of module.

9/17/03 Rob Knight: broke out special functions into separate module.

2/12/04 Rob Knight: allowed binomial_high to take -1 as an input (i.e. 
Pr(X > -1) = Pr(X >= 0) = 1).

8/4/04 Rob Knight: round small negative floating-point values to 0 to avoid
failures from rounding errors in intermediate calculations. Made cutoff 1e-14.

2/15/05 Sandra Smit: added fprob used in case of two_tailed F test.

7/27/05 Rob Knight: added inverse distributions, and gamma distribution.

1/17/06 Sandra Smit: changed binomial_exact to use approximation with gamma
function (function ln_binomial in special.py), so it works for all floating 
point values of trials and successes as well.
"""
from __future__ import division
from old_cogent.maths.stats.special import erf, erfc, igamc, igam, betai, log1p, \
    expm1, SQRTH, MACHEP, MAXNUM, PI, ndtri, incbi, igami, fix_rounding_error,\
    ln_binomial
    #ndtri import b/c it should be here anyway
