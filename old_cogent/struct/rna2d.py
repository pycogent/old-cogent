#!/usr/bin/env python
#file evo/structures/rna2d.py

"""Provides following classes:  ###THIS IS OBSOLETE -- NEED TO CHANGE
...
Status: In rapid flux. Don't rely on interfaces!

Revision History

3/2-12/03 Rob Knight: written as part of ADBAnalysis example.

3/15-17/03 Rob Knight: extended for use in BayesFold.

4/29/03 Rob Knight: revised docstrings to meet guidelines. 
Removed mfeFold method from RNASequence (now provided in Vienna.py).
Removed explanatory text from the ADBAnalysis example.
Removed __contains__ from RNASequence and ViennaStructure, since __getitem__
was being used anyway.

5/8/03 Rob Knight: cleaned up multiline docstrings.
Fixed naming error in initialization of ViennaStructure.PairList.

6/9/03 Rob Knight: added __getslice__ to ViennaStructure.

7/21/03 Rob Knight: made sequence cmp more lenient: delegates to str cmp.
Added RNASequence.__iter__ (delegates to self.Sequence).
WARNING: Slicing does not seem to work in this version. Never tested?

7/23/03 Rob Knight: New class AlphabetString acts as a constrained string.

8/3/03 Rob Knight: simplified AlphabetString: no more class/instance method
hacks. Renamed it to ConstrainedString. Added ConstrainedList.
RNASequence now subclasses ConstrainedString: slicing now works!

8/4/03 Rob Knight: changed name of module. Cleaned up parameters.

8/22/03 Fixed classify in StructureNode.

12/9/03 Rob Knight: deleted sequence-related parts. Moved file to 
evo/rna_structure.py.

12/10-28/03 Rob Knight: rewritten from scratch. Major API changes. Moved file to
evo/structure/rna2d

1/20/04 Sandra Smit: repaired typo in Pairs.mismatches(), added validity check
to StructureString.__init__(), added user-friendly wrapper for ViennaStructure,
called Vienna (not tested yet, uses the old function 
_structure_and_energy(data)).

1/20-2/3 Sandra Smit: 
Partners: __setitem__() added check for paired item.
          toPairs() returns Pairs object now.
Pairs: toPartners() removed result[downstream]=upstream(done by Partn. itself)
       and changed order of the 'strict' loop.
       toVienna() changed order of the 'strict' loop
       Implemented nested() and hasPseudoknots()
StructureString: __new__() changed 'super' call
                 Implemented __str__
                 toPairs() changed return statement
WussStructure: changed the dictionaries (now: fromdict...)
removed _structure_and_energy, functionality in Vienna()

2/9/04 Sandra Smit: added PseudoknotRemover. Moved prune, refuse, and 
refuse_or_prune into this class. Also functionality of Pairs.nested(). 
Moved not_none to evo/util.py

2/12-15/04 Rob Knight: significant changes to StructureNode and to Helix (which
has now been renamed to Stem). StructureNode now initializes correctly from
ViennaStructure and writes out the structure string correctly; renumber() also
works.

2/24-26/04 Rob Knight: more StructureNode methods added. Totally rewrote Stem;
implementation is now correct and tested.

2/13/05 Sandra Smit: added stand-alone classify method written by Rob Knight.
This method has the same function as the classify method in StructureNode,
but it is _much_ faster. However, it doesn't perform any validation.

05 Sandra Smit: added parameter to PseudoknotRemover to see what pairs were
removed from the original Pairs object.

4/4/06 Sandra Smit: Changed Pairs.toVienna so that it now accepts any object
    that has the required length as well. Useful if you want to parse in the
    sequence that corresponds to the structure, rather than its length.
"""
from Numeric import zeros, put
from old_cogent.util.transform import make_trans, float_from_string
from old_cogent.base.tree import TreeNode
from old_cogent.util.misc import not_none, flatten
...
class PseudoknotRemover(object):
    """Returns a Pairs object without pseudoknots when called"""
    
    def __call__(self, data, rule="first",return_removed=False):
        """Removes pseudoknots from a list of pairs
        
        Data should be a Pairs object.
        The algorithm will keep two lists in memory (based on tree 
        representation of the pair list): 
            a current branch with pairs (representing seen pairs so far)
            pairs (or rest_of_pairs): list of pairs not examined yet
        """
        pairs = data.directed()
        pairs.sort()
        result = Pairs()
        ...
        while curr_pairs:
            pair = curr_pairs[0]    
            if not curr_branch:
                curr_branch.append(pair)
                del curr_pairs[0]
                continue
            else:
                c_up,c_down = curr_branch[-1]
                while pair[0] > c_down:
                    result.append(curr_branch.pop())
                    if not curr_branch:
                        break
                    else:
                        c_up,c_down = curr_branch[-1]
                if not curr_branch:
                    ...
                    if rule == 'first':
                        ref, new_br, new_p = self.refuse(curr_branch,curr_pairs)
                    else:    
                        new_br, new_p = \
                        self.prune_or_refuse(curr_branch,curr_pairs)
                    curr_branch = new_br
                    curr_pairs = new_p
                    continue
                else:
                    ...
                    del curr_pairs[0]
                    continue
        result.extend(curr_branch)
        result.sort()
        removed = [pair for pair in data if pair not in result]
        if return_removed:
            return result, removed
        return result
    
    
    def refuse(self, curr_branch, rest_of_pairs):
        """refuse() removes pairs pseudoknot by refusing invalid input.
        
        It refuses to add invalid pairs from the rest_of_pairs to the current
        branch until the first valid pair. Returns unchanged curr_branch and 
        new rest_of_pairs.

        curr_branch is the current branch of checked pairs.
        rest_of_pairs are all pairs not checked yet (ordered).
        """
        refused = 0
        c_up, c_down = curr_branch[-1]
        while rest_of_pairs:
            pair = rest_of_pairs[0]
            if pair[0] < c_down and pair[1] > c_down:
                del rest_of_pairs[0]
                refused += 1
            else:
                break
        return refused, curr_branch, rest_of_pairs

    def prune(self, curr_branch,rest_of_pairs):
        """prune() removes pseudoknots by pruning pairs from the current branch.
        
        It prunes the current branch until the first valid pair is found. 
        Returns new current branch and unchanged rest_of_pairs.
        
        curr_branch is the current branch of checked pairs.
        rest_of_pairs are all pairs not checked yet (ordered).
        """
        pruned = 0
        pair_up,pair_down = rest_of_pairs[0]
        valid = False
        while not valid and curr_branch:
            c_up,c_down = curr_branch[-1]
            if pair_up < c_down and pair_down > c_down:
                curr_branch.pop()
                pruned += 1
            else:
                valid = True
        return pruned, curr_branch, rest_of_pairs

    def prune_or_refuse(self, curr_branch,rest_of_pairs):
        """prune_or_refuse() decides to prune or refuse several basepairs.

        The criterion is to keep as much pairs as possible overall, 
        so the method with the least inpact will be chosen.
        
        curr_branch is the current branch of checked pairs.
        rest_of_pairs are all pairs not checked yet (ordered).
        """
        refused, ref_branch, ref_pairs = \
        self.refuse(curr_branch[:],rest_of_pairs[:])
        
        pruned, pr_branch, pr_pairs =\
        self.prune(curr_branch[:],rest_of_pairs[:])
        
        if pruned < refused:
            return pr_branch, pr_pairs
        else:
            return ref_branch,ref_pairs
...
class Pairs(list):
    ...
    def nested(self, rule="first"):
        """Returns copy of self omitting pseudoknotted pairs.

        rule can be either "first" (first helix formed is the 'helix'; 
        subsequent helices are the 'pseudoknot') or "majority" (keeps the
        largest number of base pairs overall).
        """
        remover = PseudoknotRemover()
        return remover(self,rule=rule)
...
class StructureNode(TreeNode):
    ...
    def __init__(self, Data=None, Children=None, Parent=None):
        ...
        super(StructureNode, self).__init__(Data, Children, Parent)
    ...
    def pairBefore(self):
        ...
        new_pair.append(self)
        #add the new pair to parent, and delete its unpaired siblings
        curr_parent.insert(curr_idx, new_pair)
        del curr_parent[curr_idx + 1]
        del curr_parent[curr_idx - 1]
        return True
    ...
    def pairAfter(self):
        ...
        new_pair = StructureNode(Data=Stem(indices[0], indices[1], 1))
        new_pair[:] = self[1:-1]    #all except start and end
        self[:] = [new_pair]
        return True
    ...
    def pairChildren(self, first, second):
        ...
        del self[second_index]
        new_node[:] = self[first_index + 1 : second_index]
        self[first_index] = new_node
        return True
