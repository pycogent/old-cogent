...
9/23/03 Rob Knight: originally written for PyEvolve.

11/5/03 Rob Knight: renamed to usage from freq. Added methods for predicting
the base, codon, positional base, or amino acid frequency from any of the 
other objects.

5/19/04 Sandra Smit: added distance function to BaseUsage.

6/8/04 Rob Knight: Separated out BaseUsageI so we can have UnsafeBaseUsage
(doesn't override built-in methods, so fast but unsafe). Added DnaBases, 
DnaCodons, etc. for future classes that default to DNA instead of RNA, but
didn't implement these. Added content() method to BaseUsage to make it 
easier to get things like GC content. Changed implementation of distance()
to accept T and U equivalently and to use slightly more efficient algorithm
for calculation.

7/19/04 Rob Knight: positionalBases now returns BaseUsage objects, not
dicts. Bug reported by Roberto Marquez.

7/13/05 Rob Knight: added functions to support P3 and fingerprint calculations,
specifically CodonUsage methdos pr2bias, positonalGC, and fingerprint. Also
added standard names for codon blocks in CodonUsage.

1/10/05 Rob Knight: added helpers for unsafe codon usage.

2/22/06 Rob Knight: added toCartesian, fromCartesian to base usage.
2/23/06 Sandra Smit: modified distance to use Cartesian coordinates directly
4/19/06 Rob Knight: removed duplicate UnsafeCodonUsage class.
"""
from __future__ import division
from math import sqrt
from old_cogent.base.stats import Freqs, Numbers, UnsafeFreqs
from old_cogent.maths.stats.special import fix_rounding_error
from old_cogent.util.array import euclidean_distance
from old_cogent.util.misc import Delegator, FunctionWrapper, InverseDict
from old_cogent.base.genetic_code import GeneticCodes, GeneticCode as GenCodeClass
from old_cogent.base.info import Info as InfoClass
from string import upper
from Numeric import array, concatenate, sum
...
class BaseUsageI(object):
    ...
    def content(self, string):
        ...
        """
        return sum([self.get(i, 0) for i in string])
