#!/usr/bin/env python
#file cogent/base/bitvector.py

"""Classes for dealing with bitvectors: lists of 1 and 0.

Owner: Jeremy Widmann jeremy.widmann@colorado.edu

Status: Development -- but should be reasonably stable
...
Written 1/8/03 by Jeremy Widmann

Revised 4/9-13/03 by Rob Knight:
- changed interface to make data private
- changed many method names and parameter lists
- rewrote most list support methods using list comprehensions
- changed most initializers to use None instead of 0 so that errors
  that fail to check each position will be caught at validation

Revised 4/15-16/03 by Rob Knight:
- deleted +, *, and - from interface (formerly synonyms for |, &, ~)
- changed internal implementation to use longs instead of lists

Revised 4/26/03 by Rob Knight:
- changed interface so now only accepts sequences
- added method to convert a nucleic acid sequence to a BitVector
- added int() and long() methods for convenience

Revised 4/29/03 by Jeremy Widmann
- added stateChanges function
- added sequenceDivision function

Revised 5/26/03 by Rob Knight
- added toNumber() method
- changed implementation of __invert__ to avoid itemwise looping
- more efficient implementation of mask creation: no more strings!
- removed caching since profiling showed it to be no benefit
- re-implemented count() with about 10x speed increase
- added rfind() method, but found that it's much slower than built-in
  string methods, so don't use!

Revised 6/3/03 by Jeremy Widmann
- fixed bug in sequenceDivision method

Revised 6/10/03 by Rob Knight
shortened sequenceDivision and stateChanges somewhat: should be more
efficient. Is it worth caching the indices in stateChanges instead
of looking them up each time?

Revised 10/12/03 by Rob Knight: Changed interface completely. Now enforce
division betwen mutable and immutable bitvectors. Bitvector() is now 
factory function that returns Bitvector implemented with short or long
as needed. Some former methods of Bitvector are now factory functions,
and others are free-standing. Fixed slicing interface so that __setitem__
and __getitem__ now work with arbitrary slices. Note: have not implemented
__delitem__, so it is not possible to change the length of a Bitvector
by slice assignment (yet).

Revised 10/14/03 Rob Knight: added VectorFromRuns and VectorFromPositions 
factory functions.

Revised 10/15/03 Rob Knight: Renamed VectorFromPositions to VectorFromSpans. 
Added VectorFromPositions (takes list of positions, not list of (start,end)
coordinates).

Revised 12/8/03 by Rob Knight: Changed short bitvector operations to coerce
the result to a long if other is a long (consistent with automatic promotion
to long in other Python numeric operations). Previously returned NotImplemented:
apparently, x.__and__(y) is not the same as x & y if x and y are int or long,
for whatever reason. Thanks to Greg Caporaso for discovering the problem.

Revised 1/23/04 by Rob Knight: Changed Bitvector constructor so that it checks
for length explicitly rather than using try/except. For some reason, this fixes
a memory leak. Presumably, Exception or Warning objects were accumulating.
"""
