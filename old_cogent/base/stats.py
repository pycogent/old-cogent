...
4/17/03 Rob Knight: Numbers now subclasses list, and Freqs
subclasses dict. Numbers now delegates stats functions to on-the-fly
Freqs objects.

4/29/03 Rob Knight: docstrings now follow guidelines. Cleaned up subclassing.

5/7/03 Rob Knight: removed [] default parameter from Numbers constructor.

5/8/03 Rob Knight: added Numbers.normalize()

5/19/03 Rob Knight: added Numbers.accumulate() and Numbers.choose().
Fixed superclass calls to correct behavior as per _Nutshell_ p 90.

5/23/03 Rob Knight: added Alphabet property to Freqs. Added the
following methods: choose, randomSequence, subset, isValid, setdefault,
update. Removed add.

5/26/03 Rob Knight: changed stdev to use multiplication, not exponentiation.
Also added ability to specify correction (e.g. for population stdev).

6/5/03 Rob Knight: added expand() to Freqs, turning it into a
list of count copies of each item. Should be the inverse of Numbers if
keys are numbers.

6/6/03 Rob Knight: fixed behavior of FrequencyDistibution.randomchoice()

8/4/03 Rob Knight: added SummaryStatistics object, and changed Numbers
and Freqs to conform to its interface.

8/6/03 Rob Knight: relaxed restrictions on SummaryStatistics.

9/4/03 Rob Knight: renamed file.

9/17/03 Rob Knight: minor docstring editing.

10/14/03 Rob Knight: Changed NumberList and FrequencyDistribution to subclass
ConstrainedList and ConstrainedDict respectively. Changed names to
Numbers and Freqs respectively (used search and replace, so also changed
in comments above). Broke out methods that required both keys and values
to be numeric into a separate class (NumberFreqs).

11/4/03 Rob Knight: Changed to inherit from MappedList and MappedDict.

11/5/03 Rob Knight: Numbers.normalize() should now work on empty list or
list whose sum is zero.

5/18/04 Sandra Smit: added scale() and round() to Freqs. Added a scaling 
possibility to Freqs.expand()

6/3/04 Rob Knight: added UnsafeNumbers, UnsafeFreqs, and UnsafeNumberFreqs
(versions that don't override built-in methods and are therefore much faster,
although more dangerous). Added stats functions to Freqs, which assume that
you want to compare categories (e.g. Variance calculates the variance of the
numbers for each key) rather than assuming that you want to calculate the
weighted statistics where the value of the key has meaning as per NumberFreqs.

Cleaned up other methods as follows:
    - fixed bug in SummaryStatistics.SumSquares (now returns correct value
      instead of None).
    - changed SummaryStatistics __cmp__ to treat missing values as 0 rather
      than ignoring them (otherwise, could find situations where __cmp__
      results were intransitive, leading to sort problems).

6/8/04 Rob Knight: Many additional changes, including additional constructors
for different kinds of input data (e.g. construction from sequence of key, val
pairs now sums the values across duplicate keys rather than replacing with the
last value read).

1/14/05 Rob Knight: added option to allow Freqs.fromTuples to use the key.

2/7/05 Rob Knight: other FreqsI constructors now also have the option to use
the key. Additionally, FreqsI now defines "newFromX" classmethods that allow
FreqsI (or its subclasses) to be constructed directly from data of the
appropriate type. Previously, this had to be done through an instance.
"""
from __future__ import division
from old_cogent.util.misc import FunctionWrapper, MappedList, MappedDict, \
    ConstraintError
...
class SummaryStatistics(object):
    ...
    def __str__(self):
        ...
        result = []
        for field in ["Count", "Sum", "Mean", "StandardDeviation", \
                       "Variance", "SumSquares"]:
            try:
                result.extend([field, '\t', str(getattr(self, field)), '\n'])
            except:
                pass
        return ''.join(result)
