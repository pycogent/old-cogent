#!/usr/bin/env python
#file cogent/base/dict2d.py
"""
Status: development
Revision History
Author: Greg Caporaso caporaso@colorado.edu

1/22/04:File Creation, creation of much matrix functionality

1/28-30/04: Organization of inheritance, creation of Matrix factory 
method

2/1-11/04: Organization, addition, and testing of many methods. Renaming
of some variable and method names as nesessary to meet coding guidelines, 
preparation for check-in to CVS

2/14/04: renamed to util.py and moved into cvsroot

3/30 - 4/4/04: complete rewrite of code based on decision to reimplement
all matrices based on List2D, Dict2D, NamedList, and NamedList2D. As 
of 4/4 all inital classes and methods are checking in and fully functional.

4/9/04: performance upgrades to many of the Dict2D methods, addition and tests
of the purge method in Dict2D, changed NamedList2D to disallow columns with
different sets of keys.

4/20/04 Rob Knight: added None as default parameter to each of the constructors,
allowing empty init.

4/22/04 Rob Knight: after looking through other code in PyEvolve, it's clear
that Dict2D should be richer and the other classes are not necessary. Made
appropriate changes.

4/26/04 Greg Caporaso: fixed parse errors, which included imported deepcopy from
copy and a few missing parenthesis, changed name of apply() to scale()

4/30/04 Rob Knight: fixed several errors in Items, Rows, Cols, and some of the
alternative initializers. Now tested up to toLists().

5/4/04 Rob Knight: finished testing, and added get*, get*If, and get*Indices
(which work similarly to the ones in the Alignment class).

5/6/04 Greg Caporaso: fixed bug that caused error to be raised in 
_guess_input_type instead of calling fromIndices() when RowOrder and/or
ColOrder are None; fixed bug in toLists() where self.RowOrder was used
rather than a local row_order which was defined -- caused an error when
self.RowOrder == None

5/7/04 Greg Caporaso: modifed init to handle a data=None parameter as an
empty dict and continue processing init -- this allows for no data be 
passed in and still have the dict get padded as necessary

6/8/04 Rob Knight: Changed default of Pad in Dict2D init from False to None
for consistency with other parameters and to allow correct subclassing.
"""
...
class Dict2D(dict):
    ...
    def toLists(self, headers=False):
        ...
        Numeric or numarray to generate a real array object for numerical
        calculations (if headers are False, the default).
        """
        col_order = self.ColOrder or self.colKeys()
        row_order = self.RowOrder or self.rowKeys()
        if self.Pad:
