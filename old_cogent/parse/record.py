...
10/8/03 Rob Knight: Written to support unigene file format parser.

10/22/03 Rob Knight: Added many methods. Changed name to 'records'. Added 
negative index handling to DelimitedSplitter.

10/23/03 Rob Knight: Added list_extender

10/26/03 Rob Knight: split GenericRecord into GenericRecord and MappedRecord
for performance reasons.

10/27/03 Rob Knight: 20% speed improvement in MappedRecord by removing 
unnecessary improvements and binding local variables. A small additional
speed increase (5%) is possible by avoiding super() and calling dict 
methods directly, but this can break inheritance so I decided the 
optimization was not worthwhile and changed it back.

11/12/03 Rob Knight: Fixed bug whereby MappedRecord would return the _same_
DefaultValue every time it was requested. Ensured that DefaultValue is 
returned by __getitem__ as well as __getattr__.

6/14/03 Rob Knight: Changed MappedRecord getattr so that it no longer gets
doubly private methods (i.e. those starting with '__') from its class. This
prevents a bug where MappedRecord instances would pretend to be class objects
because they 'implement' __bases__, which had dire consequences for pydoc.
"""
from copy import deepcopy
from old_cogent.util.misc import iterable
