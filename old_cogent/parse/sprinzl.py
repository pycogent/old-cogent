...
The OneLineSprinzlParser was the first attempt. Note that it doesn't capture
all the data. The Genomic tRNA database has a different format from the
other two files (tRNA.xls and tDNA.xls), just to make things a little more
confusing.

The Sprinzl tRNA databases can be obtained here:
    http://www.uni-bayreuth.de/departments/biochemie/trna/

Revision History

Written 10/29/03 by Rob Knight.
11/8/03 Rob Knight: updated to use Rna constructor.

1/7/06 Jeremy Widmann: added _fix_structure function.

2/14/06 Sandra Smit: added sprinzl_to_vienna (and helper functions get_pieces
and get_counts) that translate a structure in sprinzl format into Vienna
format.
"""
from old_cogent.util.misc import InverseDict
