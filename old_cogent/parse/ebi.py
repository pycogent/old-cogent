#!/usr/bin/env python
#file ebi.py: parser for ebi flat file format.
"""Provide a parser for SwissProt EBI format files.

Owners: Zongzhi Liu <zongzhi.liu@colorado.edu>
...
01/25/06 Sandra: make the base of the parser

01/28/06 Zongzhi Liu: move the codes from old ebi.py, while add DictBundle,
ListBundle, join_split_parser, join_split_dict_parser, mapping parser 
inspired by Ravi's Java classes ideas.

02/04/06 Zongzhi Liu: Documents added from ebi user Manual for individual 
parsers, ft_parser, de_parser, etc modified following the decisions on
a meeting (Rob, Sandra and Zongzhi)

02/11/06 Zongzhi Liu: LisdBundle deleted, generalize period_tail_finder into
TailedRecordFinder, join_split_parser remove chars_to_delete, DictBundle
changes to pairs_to_dict;  Documents added for XX_parsers, Reorganized
functions

02/16/06 Zongzhi Liu: format changes following sandra's suggestions; labeloff
used in each individual parser, so that a parser takes the original lines.

02/20/06 Zongzhi Liu: Minor changes made while writing tests

02/22/06 Zongzhi Liu: TailedRecordFinder moved to record_finder.py

todo --  ?, 
    use pydoc to test and improve the documents
    run agaist the sprot and trembl database

"""
#################################
# over head for debugging during development
# embed ipython 
#from IPython.Shell import IPShellEmbed
#ipshell = IPShellEmbed()
#uncaught exceptions to ipython
import sys,IPython.ultraTB
sys.excepthook = IPython.ultraTB.FormattedTB(mode='Verbose', call_pdb=1)
#good old stuff: pdb and pprint
from pdb import set_trace
from pprint import pprint, pformat
# end of debugging overhead
#################################
_debug = False
...
from old_cogent.parse.record import RecordError, FieldError
from old_cogent.util.misc import identity, curry,\
...
def join_split_dict_parser(lines, delimiters=[';', ('=',1), ','], 
    ...
    try:
        ...
        if strict:
            ...
            #warn('Failed to get a dict from pairs: %s' % pairs)
            return pairs  
...
<<<<<<< ebi.py
    viruses, only the common English name is given.  Examples:
=======
...
<<<<<<< ebi.py
ra_parser = curry(join_split_parser, 
        chars_to_strip=' ;', delimiters=',')
=======
>>>>>>> 1.13
...
def EbiParser(lines, seq_constructor=Sequence, 
    ...
    #print err rec written into 'ebi.log'
    log_file.close()
