...
"""Parsers for psi-blast and blat
...
Revision History:
7/25/05 Rob Knight: Moved into Cogent, merged with another implementation.
7/28/05 Rob Knight: Added ability to return value as well as id in 
LastProteinIds.
4/6/2006 Micah Hamady: Implemented  BlastResult 
"""
...
class BlastResult(dict):
    ...
    def __init__(self, data, blast_type="blast"):
        """
        Init using blast results

        data: blast output from the m = 9 output option
        blast_type: blast, psiblast, blat 
        ...
        elif blast_type == "blat":
            parser = MinimalBlatParser9
        else:
    ...
    def filterByField(self, iteration=-1, field='E-value', threshold=0.001, return_self=False):
        """Iterates over all queries and returns filtered hits for each 
        ...
        for q, hits in self.iterHitsByQuery(iteration=iteration):
            ...
            for hit in hits:
                ...
                if cmp_fun(new_val, threshold):
                    filtered_hits.append(hit) 

            yield q, filtered_hits
