...
#!/usr/bin/env python
#file evo/parsers/record_finder.py
...
10/9/03 Rob Knight: repackaged into current module structure.

10/22/03 Rob Knight: added LineGrouper 

10/29/03 Rob Knight: made line stripping optional by adding constructor and
skip_empty arguments to each of the RecordFinder classes and LineGrouper.

11/12/03 Rob Knight: replaced skip_empty with a more general ignore variable,
allowing arbitrary function to decide what lines to ignore.
WARNING: this means the interface has changed! Expect 'ignore' to be a function
that can be called on a line, whereas 'skip_empty' was boolean.

01/28/06 Zongzhi Liu: two parameters (strict, keep_delimiter) added to 
DelimitedRecordFinder.

02/22/06 Zongzhi Liu: add TailedRecordFinder
"""
...
def isFastaLabel(x):
    ...
    return x.startswith('>')
#The following is an example of the sorts of iterators RecordFinder returns.
FastaFinder = LabeledRecordFinder(isFastaLabel)
