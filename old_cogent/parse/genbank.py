#!/usr/bin/env python
#file genbank.py: parser for GenBank flat file format.
"""Revision History

7/15/05 Rob Knight: File created.
7/28/05 Rob Knight: now uses rstrip() to map lines, and searches for '//'
instead of '//\n'.
"""
from old_cogent.parse.record import FieldWrapper
from old_cogent.parse.record_finder import DelimitedRecordFinder, \
    LabeledRecordFinder
from old_cogent.base.genetic_code import GeneticCodes
...
def sequence_parser(lines, constructor=''.join):
...
def feature_parser(lines):
    ...
    try:
        result['location'] = \
            location_line_parser(location_line_tokenizer(location))
...
def simple_location_segment_parser(segment):
...
def location_line_parser(tokens, parser=simple_location_segment_parser):
...
def feature_table_parser(lines):
    ...
    if lines[0].startswith('FEATURES'):
        lines = lines[1:]
    return [feature_parser(f) for f in indent_splitter(lines)]
...
def source_parser(lines):
    ...
    species, taxonomy = organism_parser(lines[source_length:])
...
def locus_adaptor(lines, curr):
    curr.update(locus_parser(lines[0]))
def source_adaptor(lines, curr):
    curr.update(source_parser(lines))
def ref_adaptor(lines, curr):
    if 'references' not in curr:
        curr['references'] = []
    curr['references'].append(reference_parser(lines))
def feature_table_adaptor(lines, curr):
    if 'features' not in curr:
        curr['features'] = []
    curr['features'].extend(feature_table_parser(lines))
def sequence_adaptor(lines, curr):
    curr['sequence'] = sequence_parser(lines)
...
def location_segment_parser(location_segment):
    ...
    lsp = location_segment_parser
    lap = location_atom_parser
    ...
    #check if it's a single base reference -- but don't be fooled by accessions!
        ...
        first, second = s.split('.')
        return Location([lsp(first[1:]), lsp(second[:-1])], isRange=True)
    
    
def location_atom_parser(location_atom):
...
if __name__ == '__main__':  #demo if called from commandline
    ...
    for rec in MinimalGenbankParser(open(argv[1], 'U')):
        extract_nt_prot_seqs(rec)
