...
9/17/03 Rob Knight: originally written for PEG. Moved many of the utility
methods from BayesFold here also, as well as ConstrainedContainer.

9/20/03 Rob Knight: fixed slice handling.

10/13/03 Rob Knight: moved default parameters into classes.

10/14/03 Rob Knight: changed Delegator __setattr__ so that it checks
if the handler has the attribute before checking if the object has it. 
Reason for change is that GenericRecord pretends to have every attribute,
and hasattr() on an Delegator that forwards to GenericRecord 
always returns True (and therefore never allowed the attribute to be
set in the handler, instead setting it directly on the object through the
call to object.__setattr__ for properties of the object. 

Added Mask to ConstrainedContainer objects (incl. ValueMask for dict).
Added add_lowercase and all_chars.

10/15/03 Rob Knight: added DictFromFirst, DictFromLast, DictFromPos,
InverseDict, InverseDictMulti, trans_except, trans_all. Added maxdepth
to recursive_flatten.

10/20/03 Rob Knight: moved trans_except, trans_all to filters.py.

10/28/03 Rob Knight: added extract_delimited from Bayes.Utils to support
cogent.format.mage

10/29/03 Rob Knight: Changed AttributeForwarder to Delegator, and added warnings
about delegating to objects that pretend to have every attribute such as
MappedRecord.

10/31/03 Rob Knight: Added if_ at Micah's suggestion. NOTE: if_ always evaluates
both branches, unlike built-in if/then/else, so don't use it if one branch
might raise an exception. It's primarily useful when you want to return
one of two fixed values.

11/3/03 Rob Knight: add_lowercase now handles tuples as well as strings for
keys, primarily to support alphabet.py.

11/4/03 Rob Knight: added MappedString, MappedList, and MappedDict, which
behave like the Constrained versions except that they also use the mapping
on __contains__ and, in the case of MappedDict, __getitem__ (i.e. they behave 
as though they contained the mapped version of the item, so the item can be
looked up either by its real version or its mapped version).

Relaxed the constraints on ConstrainedDict.copy() to better handle subclasses
that don't have the same signature.

11/8/03 Rob Knight: added DistanceFromMatrix

11/24/03 Rob Knight: added find_all, find_many.

01/20/04 Amanda Birmingham: added miscellaneous functions used in BayesFold
and PPS: getNewId, generateCombinations, toString, and revComp; also error 
class NonnegIntError.

01/26/04 Amanda Birmingham: added timeLimitReached function

2/9/04 Sandra Smit: added function not_none

3/24/04 Rob Knight: made some changes to Delegator to make it copyable (in
particular, copy and deepcopy bypass the normal object constructor, so 
Delegator now checks to see if it has a _handler in __getattr__ and __setattr__,
setting _handler to None if it hasn't got one. Previous behavior was infinite
recursion.

4/8/04 Rob Knight: added PairsFromGroups to support similarity functions.

6/8/04 Rob Knight: added override of get() and has_key to MappedDict.

01/25/06 Zongzhi Liu: added curry, NestedSplitter, DictFromStr, nonstr_iterable,
modified recursive_flatten to cope with str items.

02/07/06 Zongzhi Liu: DictFromStr, nonstr_iterable, not_ removed; is_char, 
is_iterable, is_char_or_noniterable, not_list_tuple, is_str_or_noniterable, 
list_flatten added. recursive_flatten further modified to do efficient str
unpacking.

02/18/06 Zongzhi Liu: NestedSplitter modified to get rid of a bug. 
"""
...
from Numeric import logical_not
