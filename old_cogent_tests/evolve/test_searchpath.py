#!/usr/bin/env python
#file evo/test_searchpath.py
    
"""Tests private methods of SearchPath and SearchNode classes.

Revision History:
09/22/03 Amanda Birmingham: renamed from test_primer_builder.py and altered
    to test only the base SearchPath and SearchNode classes
11/04/03 Amanda Birmingham: updated tests for change in names of public
    properties: now with initial uppercase.
12/03/03 Amanda Birmingham: moved _remove_option tests to public since it is
    now a public method
12/05/03 Amanda Birmingham: changed _DEFAULT_KEY to DEFAULT_KEY; now public
12/15/03 Amanda Birmingham: moved _find_allowed_option tests to public since
    it is now a public method
01/20/04 Amanda Birmingham: changed to fit into cvsroot, rewrote imports to
    use packages.

Tests public methods of SearchPath and SearchNode.

Revision History:
09/10/03 Amanda Birmingham: created to hold tests for SearchPath and 
    SearchNode inits
09/11/03 Amanda Birmingham: added tests for SearchNode.removeOption
09/17/03 Amanda Birmingham: moved SearchNode.options into this from private
09/22/03 Amanda Birmingham: removed tests that related to variable forbidden
    length exclusion: that functionality has been moved into the PrimerPath
    subclass
11/04/03 Amanda Birmingham: updated tests for change in names of public
    properties: now with initial uppercase.
12/03/03 Amanda Birmingham: moved removeOption tests in here now that it is
    a public method
12/05/03 Amanda Birmingham: changed _DEFAULT_KEY to DEFAULT_KEY; now public
12/15/03 Amanda Birmingham: moved findAllowedOption tests in here now that it
    is a public method
01/20/04 Amanda Birmingham: changed to fit into cvsroot, rewrote imports to
    use packages.
"""
...
from old_cogent.util.misc import NonnegIntError
from old_cogent.evolve.searchpath import SearchPath, SearchNode
