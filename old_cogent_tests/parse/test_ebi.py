#!/usr/bin/env python
# evo/parsers/test_ebi.py

""" Provides tests for EbiParser and related classes and functions.

Owners: Zongzhi Liu (zongzhi.liu@gmail.com)

Status: Development

Revision History:
01/28/06 zongzhi liu: created from test_rfam.py

02/11/06 zongzhi liu: test for pairs_to_dict, join_*_parsers, 
test for TailedRecordFinder

02/20/06 Zongzhi Liu: CC parsers, FT parsers, REF parsers, Other xx_parsers and
general parsers divided into different classes;
more detailed tests added for them.

02/22/06 Zongzhi Liu: TailedRecordFinder moved to record_finder.py
todo:
    check that everyline of the code have been run by testcode.  And every bit
    of ideas has been tested.
    sequence obj from EbiParser
    ft, cc, strict?
"""

from old_cogent.parse.ebi import *
...
class EbiTests(TestCase):
    ...
    def test_period_tail_finder(self):
        ...
        self.assertRaises(FieldError,
                list, period_tail_finder(test_fail))
