...
Written 10/29/03 by Rob Knight

1/7/06 Jeremy Widmann: Added tests for _fix_structure function.
2/14/06 Sandra Smit: Added SprinzlToViennaTests to test the sprinzl_to_vienna
function and helpers.
"""
from string import strip
from old_cogent.parse.sprinzl import OneLineSprinzlParser, GenomicSprinzlParser,\
...
class OneLineSprinzlParserTests(TestCase):
    ...
    def test_minimal(self):
        ...
        self.assertEqual(result[0].Accession, 'q')
        self.assertEqual(result[0].AA, 'w')
        self.assertEqual(result[0].Anticodon, 'e')
        self.assertEqual(result[0].Species, 'r')
        self.assertEqual(result[0].Strain, 't')
    ...
    def test_init(self):
        ...
        assert first.Labels is third.Labels
        ...
        self.assertEqual(first.Labels, exp_labels)

        self.assertEqual(first.Accession, 'GA0000001')
        self.assertEqual(first.AA, 'Ala')
        self.assertEqual(first.Anticodon, 'TGC')
        self.assertEqual(first.Species, 'Haemophilus influenzae')
        self.assertEqual(first.Strain, 'Rd KW20')
        self.assertEqual(first, '-GGGGCCTTAGCTCAGCT-GGG--AGAGCGCCTGCTTTGCACGCAGGAG-------------------GTCAGCGGTTCGATCCCGCTAGGCTCCA---'.replace('T','U'))

        self.assertEqual(third.Accession, 'GA0000003')
        self.assertEqual(third.AA, 'Ala')
        self.assertEqual(third.Anticodon, 'TGC')
        self.assertEqual(third.Species, 'Chlamydia pneumoniae')
        self.assertEqual(third.Strain, 'AR39')
...
class GenomicSprinzlParserTests(TestCase):
    ...
    def test_single(self):
        ...
        self.assertEqual(s.Accession, 'GA0000001')
        self.assertEqual(s.AA, 'Ala')
        self.assertEqual(s.Anticodon, 'UGC')
        self.assertEqual(s.Species, 'Haemophilus influenzae')
        self.assertEqual(s.Strain, 'Rd KW20')
        self.assertEqual(s.Taxonomy, ['Bacteria', 'Proteobacteria', \
    ...
    def test_multi(self):
        ...
        self.assertEqual([s.Accession for s in seqs], \
            ['GA0000001', 'GA0000002', 'GA0000003'])
        self.assertEqual(seqs[2].Anticodon, 'UGC')
        self.assertEqual(seqs[0].Order, seqs[2].Order)
