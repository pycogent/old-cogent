...
Revision History
1/8/04: Greg Caporaso: File creation

1/9/04 Greg and Sandra: implemented self.RaiseError for tests where an error
should be raised. And some other small things

1/15/04 Sandra: changed test_is_structure_line to simple assert statements
in order to evaluate the output. Changed test_MRP_strict_invalid_struc to 
work with the new structure construction code. Changed test_MRP_w_valid_data
to check if the structure is an instance of WussStructure.

1/20/04 Sandra: changed the tests to test for the new features in the code 
(see rev.hist. of rfam.py).

11/29/04 Sandra: changed some of the tests to test the new return values
(sequences are now returned as Alignment object).

2/24/06 Sandra Smit: added test_RfamParser_single_family to test whether it
functions on a single family without header information. Adjusted another test
of RfamParser, because it doesn't raise an error anymore when the header is
missing.
"""

from old_cogent.parse.rfam import is_header_line, is_seq_line, is_structure_line,\
HeaderToInfo, MinimalRfamParser, RfamFinder, LabelToInfo, RfamParser,\
ChangedSequence
from old_cogent.util.unit_test import TestCase, main
from old_cogent.parse.record import RecordError
from old_cogent.parse.clustal import ClustalParser, MinimalClustalParser
...
class rfam(TestCase):
    ...
    def test_HeaderToInfo(self):
        ...
        self.assertEqual(info['DatabaseReference'],['URL; http://oberon.fvms.ugent.be:8080/rRNA/ssu/index.html;','URL; http://rdp.cme.msu.edu/html/;'])
    ...
    def test_MinimalRfamParser_strict_invalid_sequence(self):
        ...
        self.assertEqual(len(list(MinimalRfamParser(\
            self._fake_record_bad_sequence_1,strict=False))[0][1]), 3)            
    ...
    def test_LabelToInfo(self):
        """LabelToInfo: integrity of output """
        # Made up input, based on a line that would look like:
        # U17136.1/898-984  AACA..CAU..CAGAUUUCCU..GGUGUAA.CGAA
        
        s_in = 'AACA..CAU..CAGAUUUCCU..GGUGUAA.CGAA'
        #s_out = 'AACA--CAU--CAGAUUUCCU--GGUGUAA-CGAA'
        sequence = Sequence(s_in, Info={'Label':'U17136.1/898-984'})
        info = LabelToInfo(sequence)
    ...
    def test_LabelToInfo_invalid_label(self):
        """LabelToInfo: raises error on invalid label """
        s = 'AA'
        invalid_labels = ['U17136.1898-984','U17136.1/898984']
        for l in invalid_labels:
            self.assertRaises(RecordError,LabelToInfo,\
                Sequence(s,Info={'Label':l}))
        a = 'U17136.1/' #missing start/end positions
        b = '/898-984' #missing genbank id
        obs_info = LabelToInfo(Sequence(s,Info={'Label':a}))
        exp = Info({'GenBank':'U17136.1','Start':None,'End':None})
        self.assertEqual(obs_info,exp)
        obs_info = LabelToInfo(Sequence(s,Info={'Label':b}))
        ...
        obs_info = LabelToInfo(Sequence(s,Info={'Label':lab1}),\
        strict=False)
        exp = Info({'GenBank':None,'Start':None,'End':None})
        self.assertEqual(obs_info,exp)
        obs_info = LabelToInfo(Sequence(s,Info={'Label':lab2}),\
        strict=False)
        exp = Info({'GenBank':'U17136.1','Start':None,'End':None})
        self.assertEqual(obs_info,exp)
    ...
    def test_RfamParser(self):
        ...
        for r in RfamParser(self._fake_record):
            ...
            self.assertEqualItems([s.GenBank for s in sequences.iterRows()],
                [['U17136.1'],['M15749.1'],['AF090431.1']])
            self.assertEqualItems([s.Start for s in sequences.iterRows()],
                [897,154,221])
