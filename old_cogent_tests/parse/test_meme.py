#!/usr/bin/env python
#file cogent_tests/parse/test_meme.py
...
Written 6/14/04 by Jeremy Widmann.

7/13/04 Jeremy Widmann:  Put test data at end of file as a global variable for
    readability.  Changed extractCommandLineData tests according to change in
    meme.py (no longer creates a dict, just stores data as a list of strings).

7/14/04 Jeremy Widmann:  Changed self.module_info_dicts to reflect the change in
    meme.py.  Motif IDs are now parsed out of MEME output.

7/16/04 Jeremy Widmann:  Changed tests to reflect change in meme.py: Module
    instances location indices now reflect sequence indices starting with 0
    rather than 1 as the first base in a sequence.

8/18/05 Jeremy Widmann: tests to reflect change in MemeParser function to check that MEME returned Command line data and checks if Modules were found.
"""
...
class MemeTests(TestCase):
    ...
    def setUp(self):
        ...
        for module, info in zip(self.ModuleInstances, self.module_info_dicts):
            temp_module = Module({},Alphabet=DnaAlphabet,
                                 Evalue=float(info['E-value']),
                                 Llr=int(info['llr']))
            for instance in module:
                temp_module[(instance.Location.SeqId,
    ...
    def test_get_data_block(self):
        ...
        self.assertEqual(alphabet, DnaAlphabet)

    def test_get_alphabet(self):
        """getAlphabet should return the correct alphabet."""
        self.assertEqual(getAlphabet(self.alphabet_block),DnaAlphabet)
    ...
    def test_extract_module_data(self):
        ...
        for data, module in zip(self.module_data_blocks,self.Modules):
            ans = extractModuleData(data,DnaAlphabet)
