...
# test_clustalw.py
"""
Provides tests of Clustalw and module level functions in clustalw.py
...
Revision History:
9/22/04-9/26/04: Written by Sandra Smit. Clustalw _get_result_paths is not 
explicitly tested, but the expanded tests for different actions test the 
working of this method. 
The use of parameters for structure alignment are NOT tested.
...
from old_cogent.app.clustalw import Clustalw, alignUnalignedSeqsFromFile,\
...
class ClustalwTests(GeneralSetUp):
    ...
    def test_stdout_input_as_string(self):
        ...
        self.assertEqual(res['StdOut'].read(),self.stdout1)
...
class clustalwTests(GeneralSetUp):
    ...
    def test_alignTwoAlignments(self):
        ...
        res = alignTwoAlignments('/tmp/ct/align1','/tmp/ct/align2',\
            'twoalign.aln')
        self.assertEqual(res['Align'].read(),self.twoalign)
    ...
    def test_addSeqsToAlignment(self):
        ...
        res = addSeqsToAlignment('/tmp/ct/align1','/tmp/ct/seq2.txt',\
            'alignseqs')
        self.assertEqual(res['Align'].read(),self.alignseqs)
