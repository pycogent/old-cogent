#!/usr/bin/env python
#file evo/structures/test_rna2d.py
...
4/29/03 Rob Knight: revised to meet coding guidelines for BayesFold.
Removed tests for mfeFold (now part of Vienna.py)
Removed explanatory material for ADBAnalysis.

6/9/03 Rob Knight: added tests for ViennaStructure __getslice__.

7/23/03 Rob Knight: added tests for AlphabetString

8/3/03 Rob Knight: renamed AlphabetString to ConstrainedString; added tests
for ConstrainedList as well.

8/4/03 Rob Knight: changed name of module and imports.

12/9/03 Rob Knight: deleted sequence, adapted from BayesFold for PyEvolve.

Jan 04 Sandra Smit: Added tests for StructureString (both ViennaStructure
and WussStructure); still to test toTree() method. Partners and Pairs objects
fully tested. Removed test_makePairList, test_makeHelixList, and 
test_mismatches, because they were old. Tested all upper-level methods (such
as Vienna, refuse, EmptyPartners).

2/9/04 Sandra Smit: Added tests for PseudoknotRemover. Moved test_not_none to
evo/test_util.py

2/12-15/04 Rob Knight: added tests for StructureNode and Stem.

2/26/04 Rob Knight: more StructureNode tests added. Tests for Stem and 
StructureNode should now be complete.

2/13/05 Sandra Smit: added test for stand-alone classify method written
by Rob Knight. Added small test for classifying invalid structures.

2/23/06 Sandra Smit: added test for call method in PseudoknotRemover.
4/4/06 Sandra Smit: changed Pairs.toVienna test to test for parsing in
    the sequence directly rather than its length.
"""
from old_cogent.struct.rna2d import ViennaStructure,Vienna,Pairs,\
Partners,EmptyPartners,WussStructure, wuss_to_vienna, StructureNode, \
PseudoknotRemover,Stem, classify
...
class PseudoknotRemoverTests(TestCase):
    """Tests for PseudoknotRemover object"""
    
    def setUp(self):
        self.p = PseudoknotRemover()
    
    # call method is tested by Pairs ->test_nested()

    def test_refuse(self):
        """PseudoknotRemover refuse() should refuse invalid pairs list"""
        self.assertEqualItems(self.p.refuse([(0,20),(8,14)],\
        [(11,16),(12,15),(17,23)])[2],[(17,23)])
        self.assertEqualItems(self.p.refuse([(0,6),(1,5)],[(2,8),(3,7)])[2],\
        [])
        self.assertEqualItems(self.p.refuse([(0,6)],[(1,7),(9,11),(10,12)])[2],\
        [(9,11),(10,12)])
        self.assertEqualItems(self.p.refuse([(0,6)],[(1,8),(2,7)])[2],[])

    def test_prune(self):
        """PseudoknotRemover prune() should prune current branch until valid"""
        self.assertEqualItems(self.p.prune([(0,20),(8,14)],\
        [(11,16),(12,15),(17,23)])[1],[(0,20)])
        self.assertEqualItems(self.p.prune([(0,6),(1,5)],[(2,8),(3,7)])[1],[])

    def test_prune_or_refuse(self):
        """PseudoknotRemover prune_or_refuse() decides to refuse or to prune""" 
        #prune
        self.assertEqualItems(self.p.prune_or_refuse([(0,20),(8,14)],\
        [(11,16),(12,15),(17,23)]),([(0,20)],[(11,16),(12,15),(17,23)]))
        #refuse
        self.assertEqualItems(self.p.prune_or_refuse([(0,20),(8,14),(9,13),\
        (10,12)],[(11,16),(13,15),(17,23)]),([(0,20),(8,14),(9,13),(10,12)],\
        [(13,15),(17,23)]))

    def test_call(self):
        """PseudoknotRemover __call__() should work with different parameters
        """
        pairs = Pairs([(0,9),(1,8),(2,7),(3,13),(4,12),(5,11),(6,10)])
        self.assertEqual(self.p(pairs, rule="first"),[(0,9),(1,8),(2,7)])
        self.assertEqual(self.p(pairs, rule="first",return_removed=True),\
        ([(0,9),(1,8),(2,7)],[(3,13),(4,12),(5,11),(6,10)]))
        self.assertEqual(self.p(pairs, rule="majority",return_removed=True),\
        ([(3,13),(4,12),(5,11),(6,10)],[(0,9),(1,8),(2,7)]))
        #empty Pairs
        self.assertEqual(self.p(Pairs()),[])
...
class PairsTests(TestCase):
    ...
    def test_nested(self):
        """Pairs nested should remove pseudoknots according to the rules"""
                
        self.assertEqualItems(self.NoPseudo.nested(),[(1,20),(2,19),(3,7),\
        (4,6),(10,15),(11,14)])
        self.assertEqualItems(self.NoPseudo.nested('majority'),\
        [(1,20),(2,19),(3,7),(4,6),(10,15),(11,14)])

        self.assertEqualItems(self.NoPseudo2.nested(),[(1,3),(4,6)])
        self.assertEqualItems(self.NoPseudo2.nested('majority'),[(1,3),(4,6)])
        
        self.assertEqualItems(self.p0.nested(),[(0,6),(1,5)])
        self.assertEqualItems(self.p0.nested('majority'),[(0,6),(1,5)])
         
        self.assertEqualItems(self.p1.nested(),[(0,9),(5,7)])
        self.assertEqualItems(self.p1.nested('majority'),\
        [(2,12),(3,10),(5,7)])
        
        self.assertEqualItems(self.p2.nested(),[(0,10),(1,9),(5,7)])
        self.assertEqualItems(self.p2.nested('majority'),[(0,10),(1,9),(5,7)])
        
        self.assertEqualItems(self.p3.nested(),[(0,9),(1,8)])
        self.assertEqualItems(self.p3.nested('majority'),[(3,14),(4,13),(6,11)])
        
        self.assertEqualItems(self.p4.nested(),[(0,35),(2,11),(3,10),(4,9),\
        (16,28),(17,27),(18,26)])
        self.assertEqualItems(self.p4.nested('majority'),[(0,35),(2,11),\
        (3,10),(4,9),(20,33),(21,32),(22,31),(23,30)])
        
        self.assertEqualItems(self.p5.nested(),[(0,5)])
        self.assertEqualItems(self.p5.nested('majority'),[(2,8),(3,7)])
        
        self.assertEqualItems(self.p6.nested(),[(0,19),(2,6),(3,5),\
        (8,14),(9,13),(10,12)])
        self.assertEqualItems(self.p6.nested('majority'),[(2,6),(3,5),\
        (8,14),(9,13),(10,12),(16,22),(17,21)])
        
        self.assertEqualItems(self.p7.nested(),[(0,20),(2,6),(3,5),(8,14),\
        (9,10)])
        self.assertEqualItems(self.p7.nested('majority'),[(2,6),(3,5),\
        (9,10),(11,16),(12,15),(17,23),(18,22)])
