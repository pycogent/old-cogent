#!/usr/bin/env python
#file evo/test_util.py

"""Unit tests for utility functions and classes.

Currently tests:
    ClassChecker: container for instances of desired classes.
    Delegator: passes along attribute requests to a component.
...
9/20/03 Rob Knight: added tests for extended slice behavior to ConstrainedString
and ConstrainedList.

9/23/03 Rob Knight: added unreserve.

10/14/03 Rob Knight: added (limited) tests for Mask behavior on the various
ConstrainedContainer objects. Added test for add_lowercase.

10/20/03 Rob Knight: moved trans_except, trans_all to filters.py

10/28/03 Rob Knight: added tests for extract_delimited from Bayes.Utils

11/3/03 Rob Knight: added test for tuple version of add_lowercase.

11/4/03 Rob Knight: added tests for MappedString, MappedList and MappedDict,
which behave like the constrained versions except that they also convert the
items when calling __contains__, not just __setitem__. MappedDict also maps
keys on __getitem__.

11/8/03 Rob Knight: added DistanceFromMatrix.

11/12/03 Rob Knight: added caps_from_underscores.

11/24/03 Rob Knight: added tests for unflatten when preserving extra row 
pieces. Added tests for find_all and find_many.

01/20/04 Amanda Birmingham: added test classes for revComp, makeNonnegInt, 
and generateCombinations functions.

2/9/04 Sandra Smit: added test for function not_none

3/24/04 Rob Knight: added tests to ensure that copy and deepcopy work on
Delegator and the other classes. NOTE: not yet tested on Constrained* classes.

4/8/04 Rob Knight: added test for PairsFromGroups

6/8/04 Rob Knight: added tests for MappedDict get and has_key.

02/07/06 Zongzhi Liu: added tests for NestSplitter, curry, list_flatten,
is_char, is_iterable, is_char_or_iterable, is_str_or_noniterable, 
not_list_tuple; added two tests for recursive_flatten (str items, unpack str).
"""
from copy import copy, deepcopy
from old_cogent.util.unit_test import TestCase, main
# change to cogent.util.misc after check into cogent
from old_cogent.util.misc import iterable, max_index, min_index, \
