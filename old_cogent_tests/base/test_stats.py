#!/usr/bin/env python
# file cogent/base/test_stats.py

"""Tests Numbers and Freqs from cogent/base/stats.py
...
4/17/03 Rob Knight: revised to reflect increased flexibility in 
Freqs inputs.

4/29/03 Rob Knight: updated docstrings to meet guidelines.

5/7/03 Rob Knight: Numbers now accepts None in the constructor (returns
empty list).

5/8/03 Rob Knight: Added test for Numbers.normalize()

5/19/03 Rob Knight: Added tests for Numbers.accumulate() and choose()

5/22/03 Rob Knight: Added tests for Freqs initialization with 
an alphabet. Added tests for Freqs methods choice, randomSequence, subset, 
isValid.

6/5/03 Rob Knight: Added test for Freqs initialization from dict (which was 
actually OK), and test for Freqs.expand().

8/4/03 Rob Knight: Added tests for summarize().

9/4/03 Rob Knight: changed import statements.

9/17/03 Rob Knight: changed import statements.

10/14/03 Rob Knight: Added tests for NumberFreqs. Changed import statements.

10/15/03 Rob Knight: Added additional tests for normalize() and purge().

11/5/03 Rob Knight: Added test that normalize() works on empty Numbers.

5/18/04 Sandra Smit: Added tests for Freqs.scale and Freqs.round. Expanded test
    for Freqs.expand to test the scaling option.

6/3/04 Rob Knight: Cleaned up tests to reflect new division into safe and
unsafe versions of the classes (depends on validation settings). Added tests
for SummaryStatistics.

1/14/05 Rob Knight: added test for Freqs.fromTuples that uses the key.

2/4/05 Rob Knight: separated out tests of FreqsI that don't depend on changing
the keys in the dict (for compatibility with an array-based implementation
where the keys are immutable but the values can change).

2/5/05 Rob Knight: added new tests for FreqsI classmethods.
"""
from math import sqrt
from old_cogent.util.unit_test import TestCase, main
from old_cogent.base.stats import SummaryStatistics, SummaryStatisticsError,\
