...
1/22/04:File Creation

2/14/04: test completed, all run sucessfully, moved to new home in cvsroot

4/30/04 Rob Knight: tests for methods up to toLists() finished. All OK.

5/4/04 Rob Knight: tests for remaining methods. All OK.

5/6/04 Greg Caporaso: added tests of _guess_input_type(), was incorrectly
raising error when Indices data was passed in without RowOrder or ColOrder;
added additional tests of toList() which wasn't functioning when headers=True
and RowOrder and ColOrder were non-existant

5/7/04 Greg Caporaso: additional init test added to detect improper handling
of padding at times when data=None, all tests pass
"""
...
from old_cogent.base.stats import Numbers, Freqs
