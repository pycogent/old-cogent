...
Rewritten 10/12/03 by Rob Knight for new Bitvector API.

Revised 10/14/03 Rob Knight: added tests for VectorFromRuns and 
VectorFromSpans.

Revised 10/15/03 Rob Knight: added test for VectorFromPositions.

Revised 12/8/03 Rob Knight: fixed errors in bitwise operations between 
bitvectors of different sizes.
"""
from old_cogent.util.unit_test import TestCase, main
from old_cogent.base.bitvector import is_nonzero_string_char, is_nonzero_char, \
...
class BitvectorTests(TestCase):
    ...
    def test_repr(self):
        ...
        v_id = str(hex(id(v)))
        self.assertEqual(`v`, 
            '<cogent.base.bitvector.ShortBitvector object at %s>' % v_id)
        v = Bitvector(2**50, 10)
        v_id = str(hex(id(v)))
        self.assertEqual(`v`, 
            '<cogent.base.bitvector.LongBitvector object at %s>' % v_id)
...
class MutableBitvectorTests(TestCase):
    ...
    def test_repr(self):
        ...
        v_id = str(hex(id(v)))
        self.assertEqual(`v`, 
            '<cogent.base.bitvector.MutableBitvector object at %s>' % v_id)
        v = MutableBitvector(2**50, 10)
        v_id = str(hex(id(v)))
        self.assertEqual(`v`, 
            '<cogent.base.bitvector.MutableBitvector object at %s>' % v_id)
