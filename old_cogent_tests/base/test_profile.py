...
9/23/05 Sandra Smit: changed tests to test for new functionality. See comments 
profile.py.
9/28/05 Sandra Smit: Added __add__, __sub__, __mul__, __div__, 
and __truediv__. Changed tests for rowUncertainty (should return empty array 
rather than [0]). Changed test_copy and test_toOddsMatrix to tests for object
identity (assert a is b).
10/04/05 Sandra Smit: added tests for randomIndices and randomSequence.
11/9/05 Sandra Smit: commented out a test in reduce. They changed the 
array behavior in Numeric 24.1. ZeroDivisionError is no longer raised, but
the array is filled with 'nan's. Don't know how to test for this yet.
11/18/05 Sandra Smit: put test in reduce back, Profile now raises error when
inf or nan in it.
12/6/05 Sandra Smit: Added test for new option in toConsensus method.
"""
from string import translate
from __future__ import division
from Numeric import array, sum, sqrt, transpose, add, subtract, multiply,\
    divide, zeros
from RandomArray import random
from old_cogent.util.unit_test import TestCase, main
from old_cogent.base.alphabet import DnaAlphabet
from old_cogent.base.profile import Profile, ProfileError, CharMeaningProfile
...
class ProfileTests(TestCase):
    ...
    def test_init(self):
        ...
        p = Profile(array([[.1,.2],[.4,.3]]),Alphabet=DnaAlphabet,
            CharOrder="AG")
        self.assertEqual(p.CharOrder,"AG")
        assert p.Alphabet is DnaAlphabet
    ...
    def test_normalizeSequences(self):
        ...
        self.assertEqual(sum(p.Data),[1,1])
        p = self.empty_row.copy()
        p.normalizeSequences()
    ...
    def test_randomIndices(self):
        ...
        a = Alignment([p.randomIndices() for x in range(n)])
...
class ModuleLevelFunctionsTest(TestCase):
    ...
    def test_CharMeaningProfile(self):
        ...
        for obs,exp in [(p1,p1_exp),(p2,p2_exp),(p3,p3_exp),(p4,p4_exp),\
            ...
            nz = [(chr(i),r.tolist()) for i,r in enumerate(obs.Data) if r]
